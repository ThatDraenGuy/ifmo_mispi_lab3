...
public class QuadrantsBean implements Serializable {
  @Getter @Setter private List<Quadrant> quadrants;
    ...
    try {
      ...
      InputStream inputStream =
          FacesContext.getCurrentInstance()
              .getExternalContext()
              .getResourceAsStream("/WEB-INF/config.json");
      quadrants =
          mapper.readValue(inputStream, QuadrantsInfo.class).getQuadrants();
    } catch (Exception e) {
  ...
  public AttemptInfo doCheck(double x, double y, double r) {
    ...
    if (r <= 0)
      return AttemptInfo.fromHit(
          coordInfo, ShotInfo.create(false, "Invalid R value", startTime));
    for (Quadrant quadrant : quadrants) {
      boolean res = quadrant.checkHit(x, y, r);
      if (res) {
        return AttemptInfo.fromHit(
            coordInfo, ShotInfo.create(true, "That's a hit!", startTime));
      }
    }
    return AttemptInfo.fromHit(
        coordInfo, ShotInfo.create(false, "That's a miss", startTime));
  }
}
