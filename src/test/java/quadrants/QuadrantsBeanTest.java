...
public class QuadrantsBeanTest {
  ...
  public void quadrantsBeanTest() {
    ...
    quadrantsBean.setQuadrants(
        List.of(new SquareQuadrant(1, 1), new SquareQuadrant(1, -1),
                new SquareQuadrant(-1, 1), new SquareQuadrant(-1, -1)));
    ...
    assertAll(
        ...
        ()
            -> assertAll(
                ()
                    -> assertEquals(
                        quadrantsBean.doCheck(2, -4.5, 1).getCoords().getX(),
                        2),
                ()
                    -> assertEquals(
                        quadrantsBean.doCheck(2, -4.5, 1).getCoords().getY(),
                        -4.5),
                ()
                    -> assertEquals(
                        quadrantsBean.doCheck(2, -4.5, 1).getCoords().getR(),
                        1)),
        ()
        ...
        ()
            -> assertFalse(quadrantsBean.doCheck(3, -4, 5)
                               .getShot()
                               .getExecTime()
                               .isZero()),
        ()
